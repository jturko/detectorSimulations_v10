...
G4int DetectionSystemTistar::AddGasTarget(G4LogicalVolume* expHallLog)
{
    ...
    G4String target_material_name = "TistarGasTarget_" + TistarSettings::Get()->GetTargetMaterialName();
    ...
    std::cout<<"Built gas target with material \""<<target_material->GetName()<<"\", pressure "<<target_material->GetPressure()/bar*1000.<<" mbar, length "<<2.*halfThickness/cm
             <<" cm, material"<<std::endl<<"density "<<targetMatDensity/(g/cm3)<<" g/cm3, and area density "<<targetThickness/(mg/cm2)<<" mg/cm2"<<" calculated via target mass "
             <<fLogicalGasTarget->GetMass()<<std::endl;
    ...
    std::cout<<"Density calculated via ideal gas law = "
             << ((target_material->GetPressure())*(target_material->GetMassOfMolecule()*CLHEP::Avogadro)/(8.314*(joule/kelvin/mole))/(target_material->GetTemperature()))/(g/cm3)
             << " g/cm3" << std::endl;
